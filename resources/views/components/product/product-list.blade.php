                ...
                <table class="table" id="tableData">
                    ...
                    <tbody id="tableList">
                        {{-- <tr>
                            ...
                            <td>${item['unit']}</td>
                            <td>
                                <button data-id="${item['id']}" edit onclick="UpdateForm(${item['id']})"
                                    class="btn editBtn btn-sm bg-gradient-success">Edit</button>
                                <button data-id="${item['id']}" delete data-bs-toggle="modal"
                                    data-bs-target="#delete-modal"
                                    class="btn deleteBtn btn-sm bg-gradient-danger">Delete</button>
                            </td>
                        </tr> --}}
                    </tbody>
                </table>
                ...
                if (response.status === 200) {
                    ...
                    response.product.forEach(function (item, index) {
                        let row = `<tr>
                                        <td>${index + 1}</td>
                                        <td>
                                            <img class="w-15 h-auto" src="${item['img_url'] ? (item['img_url'].startsWith('http') ? item['img_url'] : 'images/product/' + item['img_url']) : 'images/default.jpg'}" alt="">
                                        </td>
                                        <td>${item['name']}</td>
                                        <td>${item['price']}</td>
                                        <td>${item['unit']}</td>
